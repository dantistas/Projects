...
const Weather = ({location}) => {
    ...
    const urlWeather = `http://api.weatherapi.com/v1/current.json?key=${api_key_weather}&q=${location}`
   ...
   useEffect(() => {
    ...
    if(!weather || !time) {
        ...
        return (
            <div>
                <p>Currently in <strong>{location}: </strong> <strong>{time.split(" ").pop()}</strong></p>
